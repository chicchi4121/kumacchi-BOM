...
export class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (this.textures.exists(TITLE_LOGO_KEY)) {
      ...
      // 対応: 以前は画面全体を一律の暗さで覆っていたため、せっかく全画面に
      // 表示したロゴ画像自体がかなり暗く・小さく見えてしまっていた。
      // 上部(画像が主役)はほぼそのままの明るさで見せ、メニューが乗る
      // 下側だけを段階的に暗くするグラデーションの暗幕に変更し、画像の
      // 見え方と選択項目の読みやすさを両立する。
      const scrim = this.add.graphics();
      scrim.fillGradientStyle(0x000000, 0x000000, 0x000000, 0x000000, 0, 0, 0.72, 0.72);
      scrim.fillRect(0, 0, this.scale.width, this.scale.height);
    }
